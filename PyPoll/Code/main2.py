...
with open(poll_csv, 'r') as csvfile:
    ...
    for row in csvreader:
        candidates[row[2]] += 1
        #make a list of all votes to add to counter
        allVotes.append(row[2])
        if row[2] not in candidateList:
            candidateList.append(row[2])

    # count number of instances of candidate votes
    for candidates in candidateList:
        print(candidates,":", str(allVotes.count(candidates))/totalVotes,"("+str(allVotes.count(candidates))+")")
